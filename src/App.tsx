...
function App() {
  ...
  const letrasErradas = chuteLetras.filter(letter => !palavraParaAcertar.includes(letter))

  const isLoser = letrasErradas.length >= 6
  const isWinner = palavraParaAcertar.split("").every(letter => chuteLetras.includes(letter))

  const addChuteLetras = useCallback((letter: string) => {
    if (chuteLetras.includes(letter) || isLoser || isWinner)
      return

    setChuteLetras(letrasAtuais => [...letrasAtuais, letter])
    ...
    <div style={{
      ...
      <div style={{ alignSelf: 'stretch' }}>
        <Keyboard
          ...
          activeLetters={chuteLetras.filter(letter => palavraParaAcertar.includes(letter)
          )}
